...
exports.refundedOrder = function(track, settings){
  ...
  transaction.cu = currency;
  transaction.tcc = track.coupon();
  transaction.pa = 'refund';
  transaction.ti = orderId;

  // event values require for enhanced ecommerce
  transaction.ev = Math.round(track.value() || track.revenue() || 0);
  transaction.el = track.proxy('properties.label') || 'event';
  transaction.ec = track.category() || 'EnhancedEcommerce';
  transaction.ea = track.event() || 'Action not defined';
  transaction.t = 'event';
  transaction.ni = track.proxy('properties.nonInteraction') || 1;

  products.map(function(origProduct, i){
    var product = new Track({ properties: origProduct });
    var productIndex = i + 1;
    transaction['pr' + productIndex + 'qt'] = product.quantity();
    transaction['pr' + productIndex + 'ca'] = product.category();
    transaction['pr' + productIndex + 'pr'] = product.price();
    transaction['pr' + productIndex + 'nm'] = product.name();
    transaction['pr' + productIndex + 'id'] = product.sku();
    if (origProduct.brand) {
      transaction['pr' + productIndex + 'br'] = origProduct.brand;
    }
    if (origProduct.variant) {
      transaction['pr' + productIndex + 'va'] = origProduct.variant;
    }
    if (origProduct.coupon) {
      transaction['pr' + productIndex + 'cc'] = origProduct.coupon;
    }
    if (origProduct.position) {
      transaction['pr' + productIndex + 'ps'] = origProduct.position;
    }
  });
  return [transaction];
};
